...
public class CommandGuild extends BasicCommand {
    ...
    public boolean onCommand(CommandSender s, Command cmd, String string, String[] args) {
        ...
        String guildName = GuildDatabaseAPI.getInstance().getGuildOf(player.getUniqueId());

        if (args.length > 0) {

            if (args[0].equalsIgnoreCase("demote")) {
                if (args.length == 2) {
                    if (GuildDatabaseAPI.getInstance().isOwner(player.getUniqueId(), guildName)) {
        ...
        if (args[0].equalsIgnoreCase("promote")) {
            if (args.length == 2) {
                if (GuildDatabaseAPI.getInstance().isOwner(player.getUniqueId(), guildName)) {
                    if (Bukkit.getPlayer(args[1]) != null) {
                        if (GuildDatabaseAPI.getInstance().isMember(Bukkit.getPlayer(args[1]).getUniqueId(), guildName)) {

                            GuildDatabaseAPI.getInstance().promotePlayer(guildName, Bukkit.getPlayer(args[1]).getUniqueId());
            ...
            if (args[0].equalsIgnoreCase("c") || args[0].equalsIgnoreCase("chat")) {
                if (args.length > 1) {
                    ...
                    GuildDatabaseAPI.getInstance().sendAlert(guildName, ChatColor.GREEN.toString() + ChatColor.BOLD + "G" + " " + GameChat.getPreMessage(player) + message.toString());
                } else {
             ...
             */
                if (args[0].equalsIgnoreCase("desc") || args[0].equalsIgnoreCase("d") || args[0].equalsIgnoreCase("motd")) {
                    ...
                    if (GuildDatabaseAPI.getInstance().isOwner(player.getUniqueId(), DatabaseAPI.getInstance().getData(EnumData.GUILD, player.getUniqueId()).toString())) {
                        if (GuildDatabaseAPI.getInstance().isOwnerOfGuild(player)) {
                            if (args.length > 1) {
                                ...
                                GuildDatabaseAPI.getInstance().setMotdOf(DatabaseAPI.getInstance().getData(EnumData.GUILD, player.getUniqueId()).toString(), motd.toString());
