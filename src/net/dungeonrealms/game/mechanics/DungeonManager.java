...
public class DungeonManager implements GenericMechanic {
				        ...
				        if ((maxAlive - monstersAlive) <= (maxAlive - NinetyPercent)) {
					        ...
					        dungeonObject.getPlayerList().stream()
		                            .forEach(player -> player.sendMessage(ChatColor.WHITE + "[" + ChatColor.GOLD
		                                    + dungeonObject.type.getBossName() + ChatColor.WHITE + "]" + " "
		                                    + ChatColor.YELLOW + "You really want to fight me?"));
				        }
			        if (dungeonObject.getPlayerList().size() <= 0
		                    || Bukkit.getWorld(dungeonObject.worldName).getPlayers().size() <= 0) {
	...
	public void updateDungeonBoard(DungeonObject dungeonObject) {
		dungeonObject.getPlayerList().stream()
		        .forEach(player -> BountifulAPI.sendActionBar(player,
		                ChatColor.AQUA + "Time: " + ChatColor.WHITE + ChatColor.GOLD
		                        + String.valueOf(dungeonObject.getTime() / 60) + "/45" + " " + ChatColor.AQUA
		                        + "Alive: " + ChatColor.WHITE + dungeonObject.aliveMonsters.size() + ChatColor.GRAY
			...
			if (player != null) {
				if (Bukkit.getPlayer(player.getUniqueId()) != null) {
					if (API.getGamePlayer(player).isInDungeon()) {
						player.sendMessage(ChatColor.WHITE + "[" + ChatColor.GOLD + dungeonObject.type.getBossName()
		                        + ChatColor.WHITE + "]" + " " + ChatColor.RED + "This instance is will close!");
	 ...
	 *
	 * @param worldName
	 *            Name of the Dungeon world(DUNGEON_unixTime)
	 * @param playerList
	 *            List of players going to Dungeon.
	 * @since 1.0
	 */
