    ...
    public void doChat(AsyncPlayerChatEvent event) {
        ...
        if (event.getMessage().startsWith("@") && !event.getMessage().contains("@i@")) {
            ...
            Bukkit.getOnlinePlayers().stream().filter(player -> player.getName().equalsIgnoreCase(playerName)).forEach(player1 -> {
                event.getPlayer().sendMessage(ChatColor.GRAY.toString() + ChatColor.BOLD + "TO " + GameChat.getPreMessage(Bukkit.getPlayer(playerName)) + event.getMessage().toLowerCase().replace("@" + playerName.toLowerCase(), ""));
                player1.sendMessage(ChatColor.GRAY.toString() + ChatColor.BOLD + "FROM " + GameChat.getPreMessage(event.getPlayer()) + event.getMessage().toLowerCase().replace("@" + event.getPlayer().getName().toLowerCase(), ""));
