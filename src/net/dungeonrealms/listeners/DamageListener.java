...
import net.dungeonrealms.monsters.MonsterStats;
import net.dungeonrealms.monsters.MonsterStats.Stats;
...
public class DamageListener implements Listener {
	...
	/**
	 * This event listens for EnderCrystal explosions. Which are buffs.. with
	 * the correct nbt at least.
	 *
	 * @param event
	 * @since 1.0
	 */
	...
	public void onBuffExplode(EntityExplodeEvent event) {
		...
		for (Entity e : event.getEntity().getNearbyEntities(radius, radius, radius)) {
			...
			e.sendMessage(new String[] { "", ChatColor.BLUE + "[BUFF] " + ChatColor.YELLOW + "You have received the "
					+ ChatColor.UNDERLINE + effectType.getName() + ChatColor.YELLOW + " buff!", "" });
		}
	}
	 ...
	 * <p>
	 * THIS METHOD IS FOR TESTING, NOT SURE IF EXTENDING THE ENTITY YOU CAN PUT
	 * ITS DROPS INSIDE. XWAFFLE. ;-)
	 *
	 * @param event
	 * @since 1.0
	 */
	...
	public void onMonsterDamagePlayer(EntityDamageByEntityEvent event) {
		...
		if (event.getEntity() instanceof Player) {
			...
			if (event.getDamager().hasMetadata("type")
					&& event.getDamager().getMetadata("type").get(0).asString().equalsIgnoreCase("mob")) {
				Stats stats = MonsterStats.getMonsterStats(nmsMonster);
	...
	public void onPlayerStrikeWithWeapon(EntityDamageByEntityEvent event) {
		if (event.getDamager() instanceof Player) {
			...
			net.minecraft.server.v1_8_R3.ItemStack nmsItem = (CraftItemStack
					.asNMSCopy(((Player) event.getDamager()).getItemInHand()));
			if (nmsItem != null && nmsItem.getTag() != null) {
				// Get the NBT of the item the player is holding.
				NBTTagCompound tag = nmsItem.getTag();
				// Check if it's a {WEAPON} the player is hitting with. Once of
				// our custom ones!
