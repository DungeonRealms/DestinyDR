    ...
    public void onPlayerUseTeleportItem(PlayerInteractEvent event) {
        ...
        if (Teleportation.canUseHearthstone(player.getUniqueId())) {
            if (!(CombatLog.isInCombat(event.getPlayer().getUniqueId()))) {
                ...
                player.sendMessage(
                        ChatColor.GREEN.toString() + ChatColor.BOLD + "HEARTHSTONE " + ChatColor.RED + "You are in combat! " + ChatColor.RED.toString() + "(" + ChatColor.UNDERLINE + Teleportation.PLAYER_TELEPORTS.get(event.getPlayer().getUniqueId()) + "s/" + ChatColor.RED + ")");
            }
        } else {
            player.sendMessage(
                    ChatColor.GREEN.toString() + ChatColor.BOLD + "HEARTHSTONE " + ChatColor.RED + "[Usage Exhausted] " + ChatColor.RED.toString() + "(" + ChatColor.UNDERLINE + Teleportation.PLAYER_TELEPORTS.get(event.getPlayer().getUniqueId()) + "s/" + ChatColor.RED + ")");
        }
    }
}
