...
public enum QueryType {
    ...
    SET_RANK("UPDATE ranks SET rank = 'DEFAULT' WHERE account_id = '%s';"),
    ...
    INSERT_BAN("INSERT IGNORE INTO punishments(account_id, type, issued, expiration, punisher_id, reason, quashed, ip_address) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s';"),
    ...
    SELECT_BANS("SELECT users.account_id, issued, expiration, punisher_id, reason, ip_address FROM punishments LEFT JOIN users ON punishments.account_id = users.account_id WHERE type = 'ban' AND users.uuid = '%s' ORDER BY expiration DESC LIMIT 1;");
