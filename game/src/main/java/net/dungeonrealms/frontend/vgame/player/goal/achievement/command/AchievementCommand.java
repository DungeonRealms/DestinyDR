package net.dungeonrealms.frontend.vgame.player.goal.achievement.command;
...
public class AchievementCommand extends BaseCommand {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (Game.getGame().getRegistryRegistry().getPlayerRegistry().getPlayer(player.getUniqueId())
                    .getData().getRankData().getRank().hasRank(EnumPlayerRank.DEV)) {
                if (args.length != 3) {
                    ...
                    if (achievement != null && player1 != null) {
                        GamePlayer gamePlayer = Game.getGame().getRegistryRegistry().getPlayerRegistry().getPlayer(player1.getUniqueId());
