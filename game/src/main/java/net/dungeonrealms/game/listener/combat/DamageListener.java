...
//            finalDamage = finalDamage - armorCalculation[0];
            ...
            if (armorReducedDamage == -1) {
                ...
                player.sendMessage(ChatColor.GOLD + "" + ChatColor.BOLD + "                      *REFLECTED* (" + ChatColor.RED + attackerName + ChatColor.GOLD + ")");
                player.getWorld().playSound(player.getLocation(), Sound.ENTITY_ZOMBIE_ATTACK_IRON_DOOR, 2F, 1.0F);
    ...
    public void onMiscDamage(EntityDamageEvent event) {
        ...
        if (event.getCause() == DamageCause.VOID || event.getCause() == DamageCause.SUFFOCATION) {
            ...
            if (event.getEntity().hasMetadata("boss") || event.getEntity().hasMetadata("easter")) {
                if (event.getEntity() instanceof CraftLivingEntity && event.getEntity().hasMetadata("boss")) {
