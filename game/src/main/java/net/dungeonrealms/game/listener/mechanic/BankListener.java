...
public class BankListener implements Listener {
    ...
    public void onPlayerPickUp(PlayerPickupItemEvent event) {
        ...
        if (ItemGem.isGem(item)) {
            ...
            if (gamePlayer != null)
                gamePlayer.getPlayerStatistics().setGemsEarned(gamePlayer.getPlayerStatistics().getGemsEarned() + gem.getGemValue());
            
            int giveGems = event.getRemaining();
            for (ItemStack pch : p.getInventory().getContents()) {
            	if (giveGems <= 0)
            		break;
    ...
    private ItemStack attemptDeposit(Player player, ItemStack item) {
    	...
    	BankMechanics.addGemsToPlayerBank(player.getUniqueId(), money.getGemValue());
    ...
    public void promptWithdrawNote(Player player) {
        ...
        Chat.listenForNumber(player, 1, Integer.MAX_VALUE, num -> {
        	...
        	if (num > ItemGemNote.MAX_SIZE) {
                ...
                DatabaseAPI.getInstance().update(player.getPlayer().getUniqueId(), EnumOperators.$INC, EnumData.GEMS, -num, true);
    ...
    public void promptUpgradeBank(Player p) {
    	int storageLevel = (Integer) DatabaseAPI.getInstance().getData(EnumData.INVENTORY_LEVEL, p.getUniqueId());
        if (storageLevel >= 6) {
        ...
        Chat.promptPlayerConfirmation(p, () -> {
        	...
        	if (success) {
        		DatabaseAPI.getInstance().update(p.getUniqueId(), EnumOperators.$SET, EnumData.INVENTORY_LEVEL, newLevel, true);
                p.sendMessage("");
