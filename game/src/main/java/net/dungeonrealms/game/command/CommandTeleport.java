...
public class CommandTeleport extends BaseCommand {
    ...
    public boolean onCommand(CommandSender sender, Command command, String s, String[] args) {
        ...
        for (EnumNamedElite elite : EnumNamedElite.values()) {
            message.addRunCommand(ChatColor.GRAY + "[" + ChatColor.GREEN + elite.name() + ChatColor.GRAY + "]" + ((i + 1) % 4 == 0 ? "\n" : " "), ChatColor.GREEN, "/tp " + elite.getLocation().getBlockX() + " " + elite.getLocation().getBlockY() + " " + elite.getLocation().getBlockZ(), "Click to teleport");
            i++;
        }
