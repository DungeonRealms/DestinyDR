...
public class ItemInventoryEvent extends FunctionalItemEvent {
    ...
    @Getter
    @Setter
    //The item that is not the DR item being activated at this point. Will be null if the click is not SWAP_WITH_CURSOR
    ...
    public ItemInventoryEvent(InventoryClickEvent evt) {
        ...
        if (evt.getAction() == InventoryAction.SWAP_WITH_CURSOR) {
            ...
            setVanillaItem(evt.getCursor());
        } else if (evt.getAction() == InventoryAction.MOVE_TO_OTHER_INVENTORY || evt.getAction() == InventoryAction.HOTBAR_MOVE_AND_READD || evt.getAction() == InventoryAction.HOTBAR_SWAP) {
